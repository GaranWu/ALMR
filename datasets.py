...
def get_imgs(mask_path, img_path, transform=None, normalize=None):
...
class TextDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data_dir.split('/')[-1] == 'celeba':
            key_int = key.split('/')[-1]
            if int(key_int) < 10:
        ...
        img_name = '%s/images/%s' % (self.data_dir, dir_key)
        mask_name = '%s/mask/%s' % (self.data_dir, mask_key)
        imgs, real_mask = get_imgs(mask_name, img_name, self.transform, normalize=self.norm)
