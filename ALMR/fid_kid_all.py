...
def get_activations(files, model, batch_size=50, dims=2048,
    ...
    Params:
    -- files       : List of image files paths      图像文件路径
    -- model       : Instance of inception model    Inception模型的实�?    -- batch_size  : Batch size of images for the model to process at once.
                     Make sure that the number of samples is a multiple of
                     the batch size, otherwise some samples are ignored. This
                     behavior is retained to match the original FID score
                     implementation.
    -- dims        : Dimensionality of features returned by Inception   从模型中返回的特征的维度
    ...
    -- A numpy array of dimension (num images, dims) that contains the
       ...
       query tensor.         返回：一个维数的numpy数组（数字图像，暗淡），其中包含给定张量与查询张量的初始值时的激活�?    """
    model.eval()
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    -- mu1   : Numpy array containing the activations of a layer of the
               ...
               for generated samples. 生成样本的激活均�?    -- mu2   : The sample mean over activations, precalculated on an
               representative data set.  源数据集的激活均值�?    -- sigma1: The covariance matrix over activations for generated samples.  生成样本的协方差矩阵
    -- sigma2: The covariance matrix over activations, precalculated on an
               representative data set.  源数据集的协方差矩阵
...
if __name__ == '__main__':  # birds flowers celeba15000
    args = parser.parse_args()
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    for i in range(15, 43):
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('2attn_zhong+mask+2d-' + str(i * 10000))
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        calculate_given_paths('./images/birds/real' + str(i * 10000), './images/birds/only' +
                              str(i * 10000), '2attn_zhong+mask+2d-' + str(i * 10000), args.batch_size,
                              './result/birds-object-fid.txt', args.gpu != '', args.dims)
